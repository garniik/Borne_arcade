...
class OllamaWrapper:
    ...
    def embed(
        ...
        *,
        ...
        Si /api/embed n'existe pas, on utilise /api/generate avec embeddings=true
        """
        try:
            # Essayer d'abord le nouvel endpoint /api/embed
            body: Dict[str, Any] = {"model": model, "input": text}
            payload = self._http_request_json("POST", "/api/embed", body=body)
        except Exception as e:
            # Si /api/embed n'existe pas, utiliser l'ancienne méthode
            print(f"  ⚠️  /api/embed non disponible, utilisation de /api/generate")
            body: Dict[str, Any] = {"model": model, "prompt": text, "embeddings": True}
            payload = self._http_request_json("POST", "/api/generate", body=body)
